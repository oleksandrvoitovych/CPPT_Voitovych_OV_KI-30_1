...
# Функція для обчислення y = sin(x)/ cos(x)
def calculate(x):
...
def write_to_text_file(filename, data):
    """Додає дані у кінець текстового файлу."""
    # Режим "a" (append) відкриває файл для дозапису
    with open(filename, "a", encoding="utf-8") as file:
        file.write("-" * 20 + "\n") # Додамо лінію-розділювач для зручності
        for x, y in data:
